...
- 기간 : 2025년 12월 08일 ~ 12월 26일 (SSAFY 1학기 관통 프로젝트)

- 배포 링크 : https://jandibook.netlify.app/
...
- **알고리즘 기반 도서 추천 시스템**
  - 팔로우 기반: 팔로잉 유저의 리뷰 활동량 분석 → 상위 5명 추출 → 중복 제외 후 최신순 노출
  - 북마크 기반: 북마크 작가 랜덤 셔플 → 알라딘 API 조회 → 상위 5권 노출

- **AI 큐레이터 (캐싱 최적화)**
  - 알라딘 API + 위키피디아 API + 독자 리뷰 30개 수집 → GPT-4o-mini 프롬프트 구성
  - 분석 결과 DB 캐싱으로 동일 도서 재호출 비용 제거

- **북트래블 병렬 처리**
  - 국가별 추천 도서를 `ThreadPoolExecutor`로 알라딘 API 병렬 검색
  - 표지 이미지 없는 도서 자동 필터링 후 최대 5권 추출

- **AI 도서 취향 테스트**
  - 선택형 설문 답변 → 키워드 변환 → DB 80권 랜덤 샘플링 → GPT-4o-nano 취향 분석
  - 맞춤 나무 유형 및 추천 도서 5권 반환 (JSON 형식)

- **잔디 히트맵 시각화**
  - 독서 활동을 점수로 환산해 일별 잔디 데이터 저장
  - GitHub 스타일 히트맵 렌더링 + 경험치 기반 레벨 시스템 연동
...
### 팀장 이승연 (Backend · AI · 디자인)
짧은 기간 안에 API 캐싱, 인증, 커뮤니티, AI 기능을 함께 구현하며  
기능 구현뿐 아니라 데이터 흐름·캐시·예외 처리 등 구조적 설계가 서비스 품질을 좌우한다는 점을 직접 체감했습니다.  
협업 효율을 높이기 위해 Notion으로 프로젝트 현황을 관리하고, 특히 API 명세를 명확하게 작성하는 데 집중했습니다.   
디자인·프론트·백엔드가 긴밀하게 연결될수록 개발 속도와 완성도가 높아진다는 것을 경험하며, 
초기 기획 단계부터 충분한 소통과 문서화로 팀 전체의 방향성을 맞추는 것이 중요하다고 느꼈습니다.  
여러 시행착오를 거쳐 프로젝트를 완성하면서 성취감과 함께 아직 채워야 할 부분이 많다는 것도 알게 된 프로젝트였습니다.  
